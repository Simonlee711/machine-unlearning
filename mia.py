...
def perform_membership_inference(model, X_train, X_test, y_train, y_test):
    """
    Perform a simple membership inference attack using model prediction probabilities.
    """
    logging.info("Performing Membership Inference Attack.")

    # Get prediction probabilities
    train_proba = model.predict_proba(X_train)
    test_proba = model.predict_proba(X_test)

    # Create labels: 1 for member (training data), 0 for non-member (test data)
    member_labels = np.ones(len(train_proba))
    non_member_labels = np.zeros(len(test_proba))

    # Combine data
    attack_X = np.concatenate([train_proba, test_proba])
    attack_y = np.concatenate([member_labels, non_member_labels])

    # Shuffle the attack dataset
    shuffled_indices = np.random.permutation(len(attack_X))
    attack_X = attack_X[shuffled_indices]
    attack_y = attack_y[shuffled_indices]

    # Split into attack training and testing sets
    ...
    # Predict on the attack test set
    ...
    attack_prec = precision_score(y_attack_test, y_attack_pred, zero_division=0)
    attack_rec = recall_score(y_attack_test, y_attack_pred, zero_division=0)
    attack_f1 = f1_score(y_attack_test, y_attack_pred, zero_division=0)
    attack_auc = roc_auc_score(y_attack_test, y_attack_proba)

    logging.info(f"Membership Inference Attack - Accuracy: {attack_acc:.4f}, Precision: {attack_prec:.4f}, Recall: {attack_rec:.4f}, F1: {attack_f1:.4f}, AUC: {attack_auc:.4f}")

    return attack_acc, attack_prec, attack_rec, attack_f1, attack_auc

def main(num_remove_percentage=5, output_csv='gbt_unlearning_results.csv'):
    ...
    for did, name in tqdm(datasets[:5], desc="Processing Datasets"):
        ...
        attack_acc, attack_prec, attack_rec, attack_f1, attack_auc = perform_membership_inference(
            model=baseline_model,
            X_train=X_train_retrained,
            X_test=X_test,
            y_train=y_train_retrained,
            y_test=y_test
        )
        ...
        results.append({
            ...
            'Membership Inference Accuracy': attack_acc,
            'Membership Inference Precision': attack_prec,
            'Membership Inference Recall': attack_rec,
            'Membership Inference F1': attack_f1,
            'Membership Inference AUC': attack_auc,
    ...
    # Save results separately for clarity (optional)
    results_df.to_csv("unlearning_with_membership_inference.csv", index=False)

    # Optional: Further analysis or visualization can be done here

###############################################################
# 5. Membership Inference Functions                          #
###############################################################
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Gradient Boosted Trees with Machine Unlearning and Membership Inference')
